...
In SampleClean \cite{wang1999sample}, we found that aggregate queries can often be answered with very high accuracy (i.e 99\%) for only a small fraction of clean data.
In the context of the data lifecycle, data scientsts often run aggregate queries e.g., the number of Chinese resturants, on a new data source to assess the quality of the data.
In \sys, we implement sampling as a logical operator that can be used for quickly prototyping and optimizing workflows on samples of data and then transfering these optimizations to full datasets.
...
\textit{Parameter Change.} Many of the physical operators in \sys have tunable parameters, whose values is often very dataset-specifc, and the user feedback gives us a way to evaluate the quality of the initial parameter choice. 
