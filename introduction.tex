...
%
The prevelance of dirty data is a fundamental barrier in modern data-driven applications, since 
...
This is due to the breadth of possible errors that are manifest in the data and the variety of options to clean the data.

In addition, applications have varying data quality requirements.
For example, in aggregate query workloads, sample-and-clean techniques may give sufficiently accurate results 
without needing to clean the entire dataset~\cite{wang1999sample}.
However, extracting and de-duplicating restaurant listings for a consumer-facing website requires cleaning every record in the entire dataset.
The user must go through the cleaning process, with little or no guidance, for each of her data sources deciding what to clean, how to clean it,
...
Extract-transform-load (ETL) systems~\cite{informatica,talend,apachefalcon}
...
There have further been tools that allow analysts to define ``data quality rules" and automatically propose corrections
to maximally satisfy these rules \cite{DBLP:conf/sigmod/DallachiesaEEEIOT13}. 
As enumerating all of the possible error modes in large data set can be a daunting task, 
a number of crowd-based~\cite{gokhale2014corleone,stonebraker2013data} systems have been proposed to relieve the data cleaning analyst of the burden of rule specification, or worse manual cleaning.

Unfortunately, existing systems are limited in three key ways.
First, the data cleaning process is inherently iterative and the batch-oriented model of existing systems results in a very slow feedback loop.
This inhibits the user's ability to rapidly prototype different data cleaning solutions (e.g., tweak and 
...
a scalable data cleaning framework designed with rapid iteration and feedback.  Our goal is to
