...
The user must go through the cleaning process with little or no guidance, deciding for each of her data sources what to clean, how to clean it,
...
In order to run queries over the restaurants, the data analyst must extract structured fields of interest (e.g. address, cuisine type), then eliminate other sources of error, such as duplicate restaurant entries or non-canonical cuisine types (e.g., `Chinese' vs. `Chinese food'). 
Though these data cleaning requirements have been specified at a logical level, there are many different possible \textit{physical} implementations of the logical operators. 
...
Even after selecting (say) a crowd-based operator, many parameters might influence the quality of the output data: the number of crowd workers who examine a given webpage, the amount each worker is paid, etc.
A priori, a data analyst has little intuition for what physical plan will be optimal in this large space. 
In practice, she will likely pick a simple default (say, a rule-based extraction), then run data through the workflow and iterate by changing parameters or choosing new physical operators until she is satisfied with the cleaned data.
...
The problem of data cleaning is not new -- data cleaning systems have been explored by both the research and industry communities since the beginning of data management.
However, no existing systems address the workflow described above from end to end.
Extract-transform-load (ETL) systems~\cite{informatica,talend,apachefalcon} which require developers to manually write data cleaning rules and execute them as long batch jobs, 
...
provide no opportunity for iteration or user feedback, inhibiting the user's ability to rapidly prototype different data cleaning solutions.
...
In this paper, we introduce \sys, a system designed to support such a workflow from end to end.
