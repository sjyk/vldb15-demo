...
the variety of ways to clean the data, and the application's resilience to dirty data.
...
Extract-transform-load (ETL) systems~\cite{informatica,talend,nadeef}\ewu{need refs}
required developers to manually wrote data cleaning rules which were executed as 
long batch jobs\ewu{is there a hadoop-era ETL ref?}.  
...
(e.g., deduplication~\cite{corleone,others}\ewu{add more examples}) that may not apply to all of the data 
